...
export default {
  ...
  data: () => ({
    ...
    pf: {},
    mainNav: process.env.mainNav
  }),
  computed: {
    subNav() {
      var currentPage = this.mainNav.find(
        route => route.path === "/" + this.$route.params.mainPageSlug + "/"
      )
      if (currentPage) {
  ...
  created() {
    ...
    // console.log("main page slug:", this.$route.params.mainPageSlug)
    // console.log(
    //   "arrow path",
    //   this.mainNav.find(
    //     route => route.path === "/" + this.$route.params.mainPageSlug + "/"
  ...
  methods: {
    getPageFields() {
      //console.log(process.env.apiUrl + this.$route.path.substr(1))
      //console.log("path substr", this.$route.path.substr(1))

      axios
        .get(process.env.apiUrl + this.$route.path.substr(1))
        .then(res => {
